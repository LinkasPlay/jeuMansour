...
int main(int argc, char* argv[]) {
    ...
    // === Initialiser le random pour les volumes ===
    srand(time(NULL));

    // === Créer la fenêtre et le renderer ===
    SDL_Window* fenetre = SDL_CreateWindow("Project Shōnen Smash",
    ...
    while (page_courante != PAGE_QUITTER) {
        if (page_courante == PAGE_MENU)
            page_courante = afficher_menu(rendu);

        else if (page_courante == PAGE_OPTIONS)
            page_courante = afficher_options(rendu, PAGE_MENU);

        else if (page_courante == PAGE_HISTOIRE)
            page_courante = afficher_histoire(rendu);

        else
            page_courante = PAGE_MENU;
    }
